...
<%@ taglib uri="http://www.springframework.org/tags" prefix="spring"%>
	...
	<body data-spy="scroll" data-target="#main-menu">
	                    ...
	                    <h1 class="standard-header">
						 ...
						 <div class="row">
                            <div class="col-md-2 col-md-offset-10">
                                <a class="btn standard-hover-effect bg-red btn-block" href="/board/write.do?MENU_CD=${MENU_CD}">
	    ...
	    <!-- jQuery -->
	    <script type="text/javascript" src="/resources/ui/js/jquery.1.11.0.min.js"></script>
	...
	this.lfn_aticleInfo =function(lv_pageNum)
	{
		lfn_service(lv_pageNum, ${LIST_LIMIT_NUM} ,${VISIBLE_PAGES}, ${TOTAL_CNT},"pagingArea","${MENU_CD}","/board/articleData.json","lfn_dataView");
	};	
	...
	this.lfn_dataView = function(lo_coreJson)
	{
		...
		if (lo_coreJson != null && lo_coreJson.length > 0)
		{
			$.each(lo_coreJson, function(k, n)
			{
				...
				lv_listHtml +="</div>                                                                     			";
				lv_listHtml +="<a href='/board/listView.do?MENU_CD="+data.MENU_CD+"&SEQ="+data.SEQ+"'><p>"+data.CONTENT_DESC+"</p></a>";
				lv_listHtml +="</div>                                                                     			";
	...
	this.lfn_aticleTopInfo = function()
	{
		var params ={
					  MENU_CD : "${MENU_CD}"
					  , START_NUM 	: 0  
		...
		var lo_coreJson = gfn_service(params,"/select/board/aticleTopData.json");
		var lv_listHtml = "";
		$("#TOP_BEST_LIST").html("");
		...
		if (lo_coreJson != null && lo_coreJson.length > 0)
		{
			$.each(lo_coreJson, function(k, n)
			{
				...
				lv_listHtml +="    <div class='col-xs-12'>                                                                     ";
				lv_listHtml +="        <h3 class='standard-article-header'><a href='/board/listView.do?MENU_CD="+data.MENU_CD+"&SEQ="+data.SEQ+"'>"+data.TITLE+"</a></h3>  ";
				lv_listHtml +="        <p><i class='fa fa-calendar'></i>"+data.UPDATE_DATETIME+"</p>       ";
			...
			$("#TOP_BEST_MORE").attr("href", "/board/listView.do?MENU_CD=${MENU_CD}");
		}
	};
